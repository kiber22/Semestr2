...
void get_data(mpz_t *p, mpz_t *q, mpz_t *g, mpz_t *b, mpz_t *A, mpz_t *B) {
  ...
  FILE *user_b;
  user_b = fopen("./mqv/1_step/user_B_p.txt", "r");
  mpz_inp_str(*p, user_b, 16);
  fclose(user_b);
  user_b = fopen("./mqv/1_step/user_B_g.txt", "r");
  mpz_inp_str(*g, user_b, 16); // Генератор
  fclose(user_b);
  user_b = fopen("./mqv/1_step/user_B_q.txt", "r");
  mpz_inp_str(*q, user_b, 16);
  fclose(user_b);
  user_b = fopen("./mqv/1_step/user_B_b.txt", "r");
  mpz_inp_str(*b, user_b, 16); // Секретное число пользователя B
  fclose(user_b);
  user_b = fopen("./mqv/1_step/user_B_A.txt", "r");
